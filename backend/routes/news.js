...
router.get("/indonesia/general", async (req, res) => {
  try {
    ...
    const filteredFeed = feed.items.filter(
      (item) => item.link?.includes("katadata.co.id/finansial"), // filter only financial news
    );
    const articles = filteredFeed.slice(0, 10).map((item) => ({
      // transform to match Finnhub news format
      headline: item.title,
      url: item.link,
